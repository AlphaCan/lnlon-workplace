...
//    IOmap[0]++;
    ...
    ec_SDOread(1, 0x1c32, 0x20,FALSE,&seze,&temp,EC_TIMEOUTRXM);
    printf("temp = %x\n",temp);
...
int SetUp(uint16 slave)
{
    ...
    u16val = 0x02;//slave-master communication model
...
//    u32val = 0x10000;//slave-master communication cycle
 //   retval += ec_SDOwrite(slave, 0x1c32, 0x0A, FALSE, sizeof(u32val), &u32val, EC_TIMEOUTRXM);
...
//                while(1);
                ...
                set_default_position(1);
                set_default_position(2);
                set_default_position(3);
...
{
    actualdata.word[0] = *(ec_slave[0].inputs + 12*slave);
    actualdata.word[1] = *(ec_slave[0].inputs + 12*slave + 1);
    return actualdata.statuswords;
}

int32_t get_position(int slave)
{
    actualdata.pos[0] = *(ec_slave[0].inputs + 12*slave  + 2);
    actualdata.pos[1] = *(ec_slave[0].inputs + 12*slave  + 3);
    actualdata.pos[2] = *(ec_slave[0].inputs + 12*slave  + 4);
    actualdata.pos[3] = *(ec_slave[0].inputs + 12*slave  + 5);
    return actualdata.positon;
}

int32_t get_velocity(int slave)
{
    actualdata.vel[0] = *(ec_slave[0].inputs + 12*slave  + 6);
    actualdata.vel[1] = *(ec_slave[0].inputs + 12*slave  + 7);
    actualdata.vel[2] = *(ec_slave[0].inputs + 12*slave  + 8);
    actualdata.vel[3] = *(ec_slave[0].inputs + 12*slave  + 9);
    return actualdata.velocity;
}

int16_t get_torque(int slave)
{
    actualdata.tor[0] = *(ec_slave[0].inputs + 12*slave  + 10);
    actualdata.tor[1] = *(ec_slave[0].inputs + 12*slave  + 11);
    return actualdata.torque;
}

void set_default_position(int slave)
{
    settargetdata.positon = get_position(slave);
    IOmap[12*slave+2] = settargetdata.pos[0];
    IOmap[12*slave+3] = settargetdata.pos[1];
    IOmap[12*slave+4] = settargetdata.pos[2];
    IOmap[12*slave+5] = settargetdata.pos[3];
}

void set_position(int slave,int speed)
{
    settargetdata.positon = get_position(slave) - speed;
//    IOmap[12*slave+2] = settargetdata.pos[0];
//    IOmap[12*slave+3] = settargetdata.pos[1];
//    IOmap[12*slave+4] = settargetdata.pos[2];
//    IOmap[12*slave+5] = settargetdata.pos[3];
     memcpy(&IOmap[12*slave+2],&settargetdata.positon,4);
//     printf("aim = %d\n",settargetdata.positon);
}
